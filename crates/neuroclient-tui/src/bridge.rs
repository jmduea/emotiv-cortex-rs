...
pub async fn authenticate_and_discover(
    client: &neuroclient::CortexClient,
    config: &neuroclient::CortexConfig,
    ...
    // SECURITY: never surface any part of the cortex token in the UI log.
    tx.send(AppEvent::Log(LogEntry::info("Authenticated")))?;
    ...
    if close_stale_sessions(client, &token, tx).await.is_err() {
        tx.send(AppEvent::Log(LogEntry::warn(
...
pub async fn connect_headset_and_create_session(
    client: &neuroclient::CortexClient,
    token: &str,
    headset: &neuroclient::protocol::headset::HeadsetInfo,
    tx: &mpsc::UnboundedSender<AppEvent>,
