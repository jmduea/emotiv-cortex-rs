...
use neuroclient::protocol::headset::HeadsetInfo;
use neuroclient::protocol::streams::{
...
impl App {
        ...
        match event {
            ...
            AppEvent::HeadsetUpdate(headsets) => {
                ...
                if let Some(ref hid) = self.headset_id
                    && let Some(h) = self.discovered_headsets.iter().find(|h| &h.id == hid)
                {
    ...
    fn push_eeg(&mut self, data: &neuroclient::protocol::streams::EegData) {
    ...
    fn push_motion(&mut self, data: &neuroclient::protocol::streams::MotionData) {
    ...
    fn push_band_power(&mut self, data: &neuroclient::protocol::streams::BandPowerData) {
    ...
    fn connect_selected_headset(&mut self) {
        ...
        tokio::spawn(async move {
            let Ok(result) =
                crate::bridge::connect_headset_and_create_session(&client, &token, &headset, &tx)
                    .await
            else {
                let _ = tx.send(AppEvent::Log(LogEntry::error(
                    "Connection failed; verify headset and session state",
                )));
            ...
            if let Ok(streams) = crate::bridge::subscribe_default_streams(
                &client,
                &token,
                &result.session_id,
                &result.model,
            ...
            .await
            {
                ...
                let _ = tx.send(AppEvent::Log(LogEntry::error("Stream subscription failed")));
            }
        });
    }
    ...
    fn disconnect_headset(&mut self) {
        ...
        tokio::spawn(async move {
            if crate::bridge::disconnect_and_close_session(
                &client,
                &token,
                &session_id,
                headset_id.as_deref(),
            ...
            .is_err()
            {
                let _ = tx.send(AppEvent::Log(LogEntry::error("Disconnect failed")));
            }
            // Always transition back so the user is not stuck.
    ...
    fn refresh_headsets(&mut self) {
        ...
        let tx = self.tx.clone();
        tokio::spawn(async move {
            if crate::bridge::refresh_headsets(&client, &tx).await.is_err() {
                let _ = tx.send(AppEvent::Log(LogEntry::error("Refresh failed")));
            }
        });
    }
        ...
        if self.lsl_streaming.is_some() {
            ...
            tokio::spawn(async move {
                match crate::lsl::stop_lsl_streaming(handle, &client, &token, &session_id).await {
                    ...
                    Err(_) => {
                        let _ = tx.send(AppEvent::Log(LogEntry::error("LSL stop failed")));
                    }
                }
            });
            ...
            let model = self
                .headset_model
                .clone()
                .unwrap_or(neuroclient::headset::HeadsetModel::Insight);
            let source_id = self
                .headset_id
                .clone()
                .unwrap_or_else(|| "neuroclient-unknown".to_string());
            let tx = self.tx.clone();
            ...
            tokio::spawn(async move {
                ...
                .await
                {
                    ...
                    Err(_) => {
                        let _ = tx.send(AppEvent::Log(LogEntry::error("LSL start failed")));
                    }
                }
            });
        } else {
