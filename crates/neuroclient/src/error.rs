...
/// Security-focused `Debug` output that never includes server text,
/// credentials, tokens, payloads, filesystem contents, or transport details.
impl std::fmt::Debug for CortexError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CortexError")
            .field("category", &self.category())
            .field("api_code", &self.api_code())
            .finish()
    }
}

impl CortexError {
    /// Stable, non-sensitive category suitable for logs and telemetry.
    #[must_use]
    pub fn category(&self) -> &'static str {
        match self {
            CortexError::ConnectionFailed { .. } => "connection_failed",
            CortexError::ConnectionLost { .. } => "connection_lost",
            CortexError::NotConnected => "not_connected",
            CortexError::AuthenticationFailed { .. } => "authentication_failed",
            CortexError::TokenExpired => "token_expired",
            CortexError::AccessDenied { .. } => "access_denied",
            CortexError::UserNotLoggedIn => "user_not_logged_in",
            CortexError::NotApproved => "not_approved",
            CortexError::LicenseError { .. } => "license_error",
            CortexError::NoHeadsetFound => "no_headset_found",
            CortexError::HeadsetInUse => "headset_in_use",
            CortexError::HeadsetError { .. } => "headset_error",
            CortexError::SessionError { .. } => "session_error",
            CortexError::StreamError { .. } => "stream_error",
            CortexError::ApiError { .. } => "api_error",
            CortexError::CortexStarting => "cortex_starting",
            CortexError::MethodNotFound { .. } => "method_not_found",
            CortexError::Timeout { .. } => "timeout",
            CortexError::RetriesExhausted { .. } => "retries_exhausted",
            CortexError::ProtocolError { .. } => "protocol_error",
            CortexError::ConfigError { .. } => "config_error",
            CortexError::WebSocket(_) => "websocket_error",
            CortexError::Tls(_) => "tls_error",
            CortexError::Io(_) => "io_error",
            CortexError::Json(_) => "json_error",
        }
    }

    /// Cortex API code when the error remains in its raw API form.
    #[must_use]
    pub fn api_code(&self) -> Option<i32> {
        match self {
            CortexError::ApiError { code, .. } => Some(*code),
            CortexError::RetriesExhausted { last_error, .. } => last_error.api_code(),
            _ => None,
        }
    }
...
mod tests {
    ...
    #[test]
    fn debug_redacts_error_details() {
        let err = CortexError::ApiError {
            code: -32_999,
        ...
        assert!(err.to_string().contains("SENTINEL-SERVER-ERROR"));
    }
