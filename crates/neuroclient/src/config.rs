...
//! 5. `~/.config/neuroclient/cortex.toml`
//! 6. Legacy `~/.config/emotiv-cortex/cortex.toml`
//!
...
///
/// ```no_run
/// use neuroclient::config::CortexConfig;
///
...
/// Manual `Debug` that redacts credential material.
///
/// `client_secret` and `license` must never appear in logs, panics,
/// or `{:?}` output; only their presence is reported.
impl std::fmt::Debug for CortexConfig {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CortexConfig")
            .field("client_id", &self.client_id)
            .field("client_secret", &"<redacted>")
            .field("cortex_url", &self.cortex_url)
            .field("license", &self.license.as_ref().map(|_| "<redacted>"))
            .field("decontaminated", &self.decontaminated)
...
impl CortexConfig {
    ...
    /// 2. `NEUROCLIENT_CONFIG` environment variable
    /// 3. Legacy `CORTEX_CONFIG` environment variable
    /// 4. `./cortex.toml`
    /// 5. `~/.config/neuroclient/cortex.toml`
    /// 6. Legacy `~/.config/emotiv-cortex/cortex.toml`
    ///
    ...
    pub fn discover(explicit_path: Option<&Path>) -> CortexResult<Self> {
        ...
        // 2–3. Preferred and legacy config-path environment variables
        for key in ["NEUROCLIENT_CONFIG", "CORTEX_CONFIG"] {
        ...
        // 5–6. Preferred and legacy user config paths
...
fn parse_toml_config(_contents: &str) -> CortexResult<CortexConfig> {
    Err(CortexError::ConfigError {
        reason: "TOML config parsing is disabled. Enable the `config-toml` feature on `neuroclient` to use CortexConfig::from_file/discover file loading.".into(),
    })
}
...
/// Platform-appropriate preferred and legacy config paths.
fn user_config_paths() -> Option<[PathBuf; 2]> {
    #[cfg(target_os = "windows")]
    {
        std::env::var("APPDATA").ok().map(|dir| {
            ...
            [
                root.join("neuroclient").join("cortex.toml"),
                root.join("emotiv-cortex").join("cortex.toml"),
            ]
        })
    }
    #[cfg(not(target_os = "windows"))]
    {
        std::env::var("HOME").ok().map(|dir| {
            let root = PathBuf::from(dir).join(".config");
            [
                root.join("neuroclient").join("cortex.toml"),
                root.join("emotiv-cortex").join("cortex.toml"),
            ]
...
"#
    ...
    fn test_debug_redacts_credentials() {
        let mut config = CortexConfig::new("visible-client-id", "SENTINEL-SECRET-123");
        config.license = Some("SENTINEL-LICENSE-456".into());

        let debug = format!("{config:?}");
        assert!(
            !debug.contains("SENTINEL-SECRET-123"),
            "client_secret leaked into Debug output: {debug}"
        );
        assert!(
            !debug.contains("SENTINEL-LICENSE-456"),
            "license leaked into Debug output: {debug}"
        );
        assert!(debug.contains("<redacted>"));
        // Non-secret operational fields stay visible.
        assert!(debug.contains("visible-client-id"));
        assert!(debug.contains(DEFAULT_CORTEX_URL));
    }
...
"#,
    ...
    fn test_discover_search_priority() {
        ...
        let (home_config, legacy_home_config) = {
            #[cfg(target_os = "windows")]
            {
                set_env_var("APPDATA", &home_root);
                (
                    home_root.join("neuroclient").join("cortex.toml"),
            ...
            #[cfg(not(target_os = "windows"))]
            {
                ...
                let root = home_root.join(".config");
                (
                    root.join("neuroclient").join("cortex.toml"),
                    root.join("emotiv-cortex").join("cortex.toml"),
                )
        ...
        fs::create_dir_all(legacy_home_config.parent().unwrap()).unwrap();
        write_minimal_config(
            &legacy_home_config,
            "legacy-home-id",
            "legacy-home-secret",
...

        {
            ...
            let via_legacy_env_pointer = CortexConfig::discover(None).unwrap();
            assert_eq!(via_legacy_env_pointer.client_id, "env-file-id");

            set_env_var("NEUROCLIENT_CONFIG", env_path.to_string_lossy().to_string());
            write_minimal_config(
            ...
            remove_env_var("NEUROCLIENT_CONFIG");
            ...
            let via_legacy_home = CortexConfig::discover(None).unwrap();
            assert_eq!(via_legacy_home.client_id, "legacy-home-id");

            fs::remove_file(&legacy_home_config).unwrap();
