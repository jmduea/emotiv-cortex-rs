...
mod tests {
    ...
    fn test_insight_channels() {
        ...
        assert_eq!(model.num_channels(), 5);
        assert!((model.sampling_rate_hz() - 128.0).abs() < f64::EPSILON);

        let config = model.channel_config();
        assert_eq!(config.channels.len(), 5);
        assert!((config.sampling_rate_hz - 128.0).abs() < f64::EPSILON);
    ...
    fn test_epocx_channels() {
        ...
        assert_eq!(model.num_channels(), 14);
        assert!((model.sampling_rate_hz() - 256.0).abs() < f64::EPSILON);

        let config = model.channel_config();
        assert_eq!(config.channels.len(), 14);
        assert!((config.sampling_rate_hz - 256.0).abs() < f64::EPSILON);
    }
