...
impl ResilientClient {
    ...
    ///
    /// # Errors
    /// Returns an error if any requested route already exists or a stream
    /// route registry lock is poisoned.
    pub async fn create_stream_channels(
        &self,
        session_id: &str,
        streams: &[&str],
    ) -> CortexResult<crate::client::StreamReceivers> {
        self.client()
            .await
            .create_stream_channels(session_id, streams)
    }

    /// Subscribe to data streams.
    ///
    /// Returns the typed per-stream outcome; check
    /// [`SubscribeResult::failure`](crate::protocol::streams::SubscribeResult::failure)
    /// for streams that could not be subscribed (a partial failure is
    /// not an RPC error).
    ///
    /// # Errors
    /// Returns any error produced by the underlying Cortex API call,
    ...
    /// Returns the typed per-stream outcome; check
    /// [`SubscribeResult::failure`](crate::protocol::streams::SubscribeResult::failure)
    /// for streams that could not be unsubscribed.
    ///
    /// # Errors
    /// Returns any error produced by the underlying Cortex API call,
    /// including connection, authentication, protocol, timeout, and configuration errors.
