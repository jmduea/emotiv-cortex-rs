...
/// One confirmed stream subscription, as documented for the `subscribe`
/// and `unsubscribe` result `success` arrays.
#[derive(Debug, Clone, Deserialize)]
pub struct SubscriptionSuccess {
    /// Name of the stream (e.g. `"eeg"`, `"mot"`).
    ...
    /// Column layout for the stream's data arrays. Entries may be plain
    /// column names or nested groups (e.g. the `dev` stream nests its
    /// contact-quality columns).
    #[serde(default)]
    pub cols: Vec<serde_json::Value>,
}

/// One failed stream subscription from the `failure` array.
#[derive(Clone, Deserialize)]
pub struct SubscriptionFailure {
    /// Name of the stream that could not be (un)subscribed.
    #[serde(rename = "streamName")]
    pub stream_name: String,

    /// Cortex error code for the failure.
    #[serde(default)]
    pub code: i32,
...
impl std::fmt::Debug for SubscriptionFailure {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SubscriptionFailure")
...
/// A `success` entry: the documented object form, or a bare stream-name
/// string for compatibility with non-conforming server variants.
#[derive(Debug, Clone, Deserialize)]
#[serde(untagged)]
pub enum SubscriptionAck {
    /// Documented object form with `streamName`/`cols`/`sid`.
    Detailed(SubscriptionSuccess),
    /// Bare stream name.
    StreamName(String),
}

impl SubscriptionAck {
    /// Name of the acknowledged stream.
    #[must_use]
    pub fn stream_name(&self) -> &str {
        match self {
            SubscriptionAck::Detailed(success) => &success.stream_name,
            SubscriptionAck::StreamName(name) => name,
        }
    }

    /// Column layout, when the detailed form was returned.
    #[must_use]
    pub fn cols(&self) -> Option<&[serde_json::Value]> {
        match self {
            SubscriptionAck::Detailed(success) => Some(&success.cols),
            SubscriptionAck::StreamName(_) => None,
        }
    }
}

/// Typed result of a `subscribe` or `unsubscribe` call.
///
/// Cortex reports per-stream outcomes inside a successful JSON-RPC
/// response: streams that worked appear in `success`, streams that did
/// not appear in `failure`. A non-empty `failure` array is **not** a
/// transport error — callers must check it.
#[derive(Debug, Clone, Default, Deserialize)]
pub struct SubscribeResult {
    /// Streams that were successfully (un)subscribed.
    #[serde(default)]
    pub success: Vec<SubscriptionAck>,

    /// Streams that failed, with per-stream error details.
    #[serde(default)]
    pub failure: Vec<SubscriptionFailure>,
}

impl SubscribeResult {
    /// Build a full-success result for legacy unsubscribe responses that
    /// acknowledge the request with a message instead of per-stream arrays.
    pub(crate) fn all_success(streams: &[&str]) -> Self {
        Self {
            success: streams
                .iter()
                .map(|stream| SubscriptionAck::StreamName((*stream).to_string()))
                .collect(),
            failure: Vec::new(),
        }
    }

    /// Returns `true` if `stream` appears in the `success` array.
    #[must_use]
    pub fn confirms(&self, stream: &str) -> bool {
        self.success.iter().any(|ack| ack.stream_name() == stream)
    }

    /// Returns `true` if `stream` is acknowledged for `session_id`.
    ///
    /// Detailed acknowledgements with a session ID must match the requested
    /// session. Bare stream-name acknowledgements and detailed legacy entries
    /// without `sid` remain accepted for compatibility.
    #[must_use]
    pub fn confirms_for_session(&self, stream: &str, session_id: &str) -> bool {
        self.success.iter().any(|ack| match ack {
            SubscriptionAck::Detailed(success) => {
                success.stream_name == stream
                    && success.sid.as_deref().is_none_or(|sid| sid == session_id)
            }
            SubscriptionAck::StreamName(name) => name == stream,
        })
    }

    /// The failure entry for `stream`, if it failed.
    #[must_use]
    pub fn failure_for(&self, stream: &str) -> Option<&SubscriptionFailure> {
        self.failure
            .iter()
            .find(|failure| failure.stream_name == stream)
    }

    /// The success entry for `stream`, if it succeeded.
    #[must_use]
    pub fn ack_for(&self, stream: &str) -> Option<&SubscriptionAck> {
        self.success.iter().find(|ack| ack.stream_name() == stream)
    }
}
...
mod tests {
    ...
    #[test]
    fn subscription_failure_debug_redacts_server_message() {
        let failure = SubscriptionFailure {
            stream_name: "eeg".into(),
            code: -32_999,
            message: "SENTINEL-SERVER-ERROR".into(),
        };

        let debug = format!("{failure:?}");
        assert!(!debug.contains("SENTINEL-SERVER-ERROR"));
        assert!(debug.contains("-32999"));
    }
    ...
    fn test_parse_eeg_data_insight() {
        ...
        let data = EegData::from_eeg_array(&eeg, 5, 1_609_459_200.0).unwrap();
    ...
    fn test_parse_device_quality_insight() {
        ...
        assert!((quality.signal_strength - 1.0).abs() < f32::EPSILON);
    ...
    fn test_parse_motion_data() {
        ...
        let motion = MotionData::from_mot_array(&mot, 1_609_459_200.0).unwrap();
    ...
    fn test_parse_band_power() {
        ...
        let bp = BandPowerData::from_pow_array(&pow, 5, 1_609_459_200.0).unwrap();
