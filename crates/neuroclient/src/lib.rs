...
//! `NeuroClient` is not affiliated with, sponsored by, or endorsed by Emotiv.
//!
//! ## Quick Start
//!
//! ```no_run
...
compile_error!(
    "neuroclient requires exactly one TLS backend feature: enable `rustls-tls` or `native-tls`."
);

#[cfg(all(feature = "rustls-tls", feature = "native-tls"))]
compile_error!(
    "neuroclient requires exactly one TLS backend feature: `rustls-tls` and `native-tls` are mutually exclusive."
);
