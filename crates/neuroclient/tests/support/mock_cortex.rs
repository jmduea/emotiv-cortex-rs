...
impl MockConnection {
    ...
    /// Wait until the client side closes this connection (drains and
    /// discards any remaining inbound requests along the way).
    pub async fn wait_for_close(&mut self) {
        loop {
            let Ok(maybe_request) = timeout(STEP_TIMEOUT, self.request_rx.recv()).await else {
                panic!("timed out waiting for the client to close the connection");
            };
