...
use neuroclient::protocol::profiles::ProfileAction;
use neuroclient::protocol::training::DetectionType;
...
async fn auth_example_workflow_matches_mocked_rpc_sequence() {
    let Some(mut server) =
        start_server_or_skip("auth_example_workflow_matches_mocked_rpc_sequence").await
    else {
        return;
    };
...
#[tokio::test]
// End-to-end workflow scenario; the RPC sequence must stay in one place.
#[allow(clippy::too_many_lines)]
async fn records_example_workflow_matches_mocked_rpc_sequence() {
    let Some(mut server) =
        start_server_or_skip("records_example_workflow_matches_mocked_rpc_sequence").await
    else {
        return;
    };
    let config = test_config(server.ws_url());
        ...
        connection
            .send_result(
                ...
                json!({
                    "record": {
                        "uuid": "record-1",
                        "title": "neuroclient example recording",
    ...
    let record = client
        .create_record(&token, &session.id, "neuroclient example recording")
        .await
        .unwrap();
...
async fn motion_example_workflow_matches_documented_subscribe_and_sample() {
    let Some(mut server) =
        start_server_or_skip("motion_example_workflow_matches_documented_subscribe_and_sample")
            .await
    else {
        return;
    };
    ...
    assert!((motion.accelerometer[0] - 0.948_257).abs() < f32::EPSILON);
...
async fn performance_metrics_example_workflow_matches_documented_sample() {
    let Some(mut server) =
        start_server_or_skip("performance_metrics_example_workflow_matches_documented_sample")
            .await
    else {
        return;
    };
    ...
    let responder = tokio::spawn(async move {
        ...
        connection
            .push_event(json!({
                "met": [false, null, false, null, null, false, null, true, 0.266_589, false, null, true, 0.098_421],
                "sid": SESSION_ID,
                "time": 1_559_903_137.174_1
            }))
            .await;
    ...
    assert_eq!(metrics.relaxation, Some(0.266_589));
    assert_eq!(metrics.attention, Some(0.098_421));
...
async fn mental_commands_example_workflow_matches_documented_sample() {
    let Some(mut server) =
        start_server_or_skip("mental_commands_example_workflow_matches_documented_sample").await
    else {
        return;
    };
    let config = test_config(server.ws_url());
