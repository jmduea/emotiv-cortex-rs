use neuroclient::protocol::headset::{ConfigMappingRequest, ConfigMappingResponse};
use neuroclient::protocol::profiles::ProfileAction;
use neuroclient::protocol::records::{ExportFormat, UpdateRecordRequest};
use neuroclient::protocol::subjects::{QuerySubjectsRequest, SubjectRequest};
use neuroclient::protocol::training::{
...
use neuroclient::{CortexClient, ResilientClient};
...
// One match arm per endpoint contract; splitting would obscure the 1:1
// mapping between StepKind variants and API calls.
...
pub(super) async fn execute_cortex_step(client: &CortexClient, kind: &StepKind) {
    match kind {
        ...
        StepKind::SubscribeStreams => {
            ...
            for stream in CONTRACT_STREAMS {
                assert!(
                    result.confirms(stream),
                    "missing success entry for {stream}"
                );
            }
        }
