...
use neuroclient::{CortexClient, CortexConfig, ResilientClient};
use serde_json::json;
...
async fn cortex_client_endpoint_contracts_table_driven() {
    let Some(mut server) =
        start_server_or_skip("cortex_client_endpoint_contracts_table_driven").await
    else {
        return;
    };
...
async fn resilient_client_endpoint_contracts_table_driven() {
    let Some(mut server) =
        start_server_or_skip("resilient_client_endpoint_contracts_table_driven").await
    else {
        return;
    };
