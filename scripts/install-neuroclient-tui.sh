...
install_root="${NEUROCLIENT_INSTALL_ROOT:-${EMOTIV_CLI_INSTALL_ROOT:-${CARGO_HOME:-$HOME/.cargo}}}"
...
if [[ "${enable_lsl}" == "true" ]]; then
  ...
  echo "Installing neuroclient-tui (with LSL) to: ${install_root}/bin"
else
  echo "Installing neuroclient-tui to: ${install_root}/bin"
  echo "  Tip: use --lsl to enable Lab Streaming Layer support (Windows/macOS)"
fi

cargo install \
  --path "${repo_root}/crates/neuroclient-tui" \
  --root "${install_root}" \
  --force \
...
case ":$PATH:" in
  ...
  *)
    echo
    echo "Add this to your shell profile to run neuroclient-tui from anywhere:"
